...
\begin{tabularx}{\textwidth}{ccX}
  ...
  0.6 & Mayo 2026    & Empaquetado del agente como binario autocontenido con PyInstaller. Script de compilación \texttt{build.py} con spec de PyInstaller multiplataforma. \\
  0.7 & Mayo 2026    & Integración de Tauri 2 en el frontend. La interfaz web se empaqueta también como aplicación de escritorio nativa multiplataforma (Windows, Linux, macOS). \\
...
\begin{enumerate}
  ...
  \item \textbf{Capa de escritorio (Tauri):} Opcionalmente, el frontend React puede distribuirse también como aplicación de escritorio nativa mediante Tauri 2, sin necesidad de instalar un navegador independiente. El núcleo de la aplicación en Rust actúa como contenedor de la SPA, gestionando la ventana nativa del sistema operativo.
\end{enumerate}
...
\begin{longtable}{cp{11cm}}
  ...
  RNF-11 & El frontend debe poder distribuirse como aplicación de escritorio nativa (Windows, Linux, macOS) mediante Tauri 2, además de como SPA web. \\
  RNF-12 & El agente cliente debe poder compilarse como un binario autocontenido (sin dependencias externas) usando PyInstaller, para facilitar su distribución masiva en los equipos del aula. \\
  \bottomrule
\end{longtable}
...
% ── Monigote UML (stick figure) ──────────────────────────────────────────────
\tikzset{stickman/.pic={
  \draw[line width=1.3pt] (0, 0.38) circle(0.22cm);
  \draw[line width=1.3pt] (0, 0.16) -- (0,-0.28);
  \draw[line width=1.3pt] (-0.30,-0.04) -- (0.30,-0.04);
  \draw[line width=1.3pt] (0,-0.28) -- (-0.24,-0.65);
  \draw[line width=1.3pt] (0,-0.28) -- ( 0.24,-0.65);
}}

% ── Actores (notación UML: monigote + etiqueta) ───────────────────────────────
\coordinate (admin) at (-7, -0.15);
\pic at (-7, 0.35) {stickman};
\node[font=\small\bfseries, text width=2.5cm, align=center] at (-7, -1.0) {Administrador};

\coordinate (prof) at (-7, -7.15);
\pic at (-7, -6.65) {stickman};
\node[font=\small\bfseries, text width=2.5cm, align=center] at (-7, -8.0) {Profesor};

\coordinate (agent) at (9, -2.15);
\pic at (9, -1.65) {stickman};
\node[font=\small\bfseries, text width=2.5cm, align=center] at (9, -3.0) {Agente\\(Sistema)};

\coordinate (mikrotik) at (9, -6.15);
\pic at (9, -5.65) {stickman};
\node[font=\small\bfseries, text width=2.5cm, align=center] at (9, -7.0) {MikroTik\\(Externo)};
...
\subsubsection{Frontend (React 19 + Vite + Material UI 7 + Tauri 2)}
...
\begin{itemize}
  ...
  \item \textbf{Vite (rolldown-vite):} Bundler de nueva generación con HMR instantáneo. Se utiliza la variante \texttt{rolldown-vite} (v7.2.5) que incorpora el nuevo motor Rolldown escrito en Rust para tiempos de compilación aún más rápidos.
  ...
  \item \textbf{Tauri 2:} Framework para empaquetar aplicaciones web como aplicaciones de escritorio nativas. Permite distribuir el frontend como un ejecutable nativo de Windows, Linux o macOS, aprovechando el WebView del sistema operativo y añadiendo un núcleo Rust ligero.
\end{itemize}
...
\node[relationship] (r5) at (16.5, 0) {restringido\_a};
...
\draw[arrow] (classroom) -- (r5) node[midway, above, card] {1};
\draw[arrow] (r5) -- (allowed) node[midway, above, card] {N};
...
\savebox{\boxAH}{\scriptsize%
  ...
  name : varchar(100) \\
  \textbf{FK} classroom\_id : bigint (NULL) \\
  \hline
  \end{tabular}%
}
...
% ── R11  devices_classroom ||--o{ devices_allowedhost ──
\path (CL.east) ++(0,-0.5) coordinate (R11a);
\path (AH.west) ++(0,-0.5) coordinate (R11b);
\draw[rel] (R11a) -- (R11b);
\cfOneRight{R11a}
\cfManyLeft{R11b}
\cfCirc{$(R11b)+(-0.14,0)$}
...
\begin{tabularx}{\textwidth}{llX}
  ...
  classroom\_id & BIGINT       & FK $\to$ devices\_classroom.id, NULL (opcional) \\
  \bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \texttt{devices\_classroom} $\longleftrightarrow$ \texttt{devices\_allowedhost}: 1:N (un aula puede tener hosts permitidos específicos; si el host no tiene aula asignada, es global).
...
\begin{longtable}{p{4.5cm}p{10.5cm}}
  ...
  \addlinespace
  \texttt{devices\_allowedhost} & IP de un host que siempre será accesible aunque se bloquee internet. Cuando se crea una regla de bloqueo, se añaden reglas \texttt{accept} para cada host permitido antes de la regla \texttt{drop}. Si tiene un aula asignada (\texttt{classroom\_id}), solo se aplica en las reglas de bloqueo de esa aula; si no tiene aula (valor nulo), se aplica de forma global en cualquier regla de bloqueo. \\
  \addlinespace
...
\begin{longtable}{llp{7cm}}
  ...
  \addlinespace
  Escritorio & Tauri 2 (Rust) & Framework para empaquetar la SPA de React como aplicación de escritorio nativa. Utiliza el WebView del sistema operativo (WebView2 en Windows, WebKitGTK en Linux, WKWebView en macOS) con un núcleo Rust mínimo. Es la alternativa moderna a Electron, con un tamaño de binario mucho menor. \\
  \addlinespace
  Empaquetado agente & PyInstaller & Herramienta para compilar el agente Python y todos sus scripts auxiliares en un único binario autocontenido. Permite distribuir el agente sin necesidad de instalar Python ni dependencias en los equipos del aula. \\
  \bottomrule
\end{longtable}
...
|   +-- requirements.txt
|-- frontend/                # React 19 + Vite + Material UI 7 + Tauri 2
|   |-- src/
...
|   |-- src-tauri/           # Nucleo Rust de la app de escritorio
|   |   |-- tauri.conf.json  # Configuracion Tauri (ventana, bundle, etc.)
|   |   |-- Cargo.toml       # Dependencias Rust (tauri 2, serde, log)
|   |   |-- src/             # Codigo Rust del nucleo (main.rs, lib.rs)
|   |   |-- capabilities/    # Permisos Tauri 2
|   |   +-- icons/           # Iconos de la aplicacion (.png, .icns, .ico)
|   |-- init-tauri.sh        # Script de inicializacion automatica de Tauri
|   +-- package.json
...
|   |-- requirements.txt
|   +-- build/               # Sistema de compilacion con PyInstaller
|       |-- build.py         # Script de compilacion multiplataforma
|       +-- netmanagement_installer.spec  # Spec de PyInstaller
...
\subsubsection{Compilación como binario autocontenido (PyInstaller)}

Para facilitar la distribución masiva del agente en los equipos del aula, se ha creado un sistema de compilación basado en \textbf{PyInstaller}. El directorio \texttt{client/build/} contiene:

\begin{itemize}
  \item \textbf{\texttt{build.py}:} Script de compilación que detecta automáticamente la plataforma (Windows o Linux), invoca PyInstaller con el spec correspondiente y genera el binario en \texttt{build/dist/\{platform\}/}.
  \item \textbf{\texttt{netmanagement\_installer.spec}:} Spec de PyInstaller que empaqueta \texttt{install\_service.py} como punto de entrada e incluye como datos adicionales los ficheros \texttt{client.py}, \texttt{config.py}, \texttt{system\_info.py} y \texttt{requirements.txt}, necesarios en tiempo de ejecución. El resultado es un ejecutable único \texttt{NetManagement-Installer} (\texttt{.exe} en Windows) que puede distribuirse mediante GPO o copiado manual sin necesidad de instalar Python en el equipo destino.
\end{itemize}

\begin{lstlisting}[language=Python, caption={Fragmento del script de compilación build.py.}]
cmd = [
    "pyinstaller",
    str(SPEC_FILE),
...
subprocess.run(cmd, check=True, cwd=str(CLIENT_DIR))
\end{lstlisting}
...
\subsection{Aplicación de escritorio — Tauri 2}

Además de funcionar como SPA web, el frontend de NetManagement puede distribuirse como una \textbf{aplicación de escritorio nativa} gracias a \textbf{Tauri 2}. Tauri es un framework que combina un núcleo en Rust con el WebView nativo del sistema operativo para renderizar la interfaz web, obteniendo binarios mucho más ligeros que Electron (que incluye Chromium completo).

\subsubsection{Estructura del proyecto Tauri}

La configuración de Tauri se encuentra en el directorio \texttt{frontend/src-tauri/}:

\begin{itemize}
  \item \textbf{\texttt{tauri.conf.json}:} Fichero principal de configuración. Define el nombre del producto (\textit{Net Management}), el identificador de la aplicación (\texttt{com.netmanagement.app}), la URL de desarrollo, la ruta de distribución y los ajustes de la ventana principal.
  \item \textbf{\texttt{Cargo.toml}:} Manifiesto de Rust. Declara las dependencias del núcleo: \texttt{tauri} (v2.10.3), \texttt{tauri-plugin-log} para logging nativo, \texttt{serde} y \texttt{serde\_json}.
  \item \textbf{\texttt{src/}:} Código Rust del núcleo. En la versión actual, el núcleo es mínimo: solo se registran los plugins y se lanza la aplicación, delegando toda la lógica de negocio al frontend React.
  \item \textbf{\texttt{capabilities/}:} Permisos y capacidades de la aplicación Tauri (modelo de seguridad basado en capabilities de Tauri 2).
  \item \textbf{\texttt{icons/}:} Iconos de la aplicación en todos los formatos requeridos (32×32, 128×128, 128×128@2x, \texttt{.icns} para macOS, \texttt{.ico} para Windows).
  \item \textbf{\texttt{init-tauri.sh}:} Script de inicialización automática que instala \texttt{@tauri-apps/cli}, ejecuta \texttt{tauri init} en modo no interactivo y parcha el fichero de configuración.
\end{itemize}

\subsubsection{Configuración de la ventana}

La ventana principal de la aplicación de escritorio se ha configurado con las siguientes características:
...
\begin{tabularx}{\textwidth}{llX}
  ...
  \midrule
  Título & Net Management & Nombre del producto. \\
  Ancho & 800 px & Anchura mínima adecuada para el dashboard. \\
  Alto & 600 px & Altura mínima adecuada. \\
  Redimensionable & No & Evita que el layout se rompa en resoluciones imprevistas. \\
  Pantalla completa & No & Modo ventana estándar. \\
  CSP & null (deshabilitado) & La app hace peticiones a un dominio externo configurable; una CSP estática rompería la funcionalidad. \\
  \bottomrule
\end{tabularx}
\caption{Configuración de la ventana Tauri de NetManagement.}
\end{table}

\subsubsection{Flujo de compilación}

El proceso para generar el instalador de escritorio es el siguiente:

\begin{enumerate}
  \item \textbf{Instalar Rust} (\texttt{rustup}) y las dependencias de sistema de Tauri (WebKitGTK en Linux).
  \item Desde \texttt{frontend/}, ejecutar: \texttt{bun run tauri build}.
  \item Tauri compila el frontend con Vite (\texttt{bun run build}), después compila el núcleo Rust y enlaza el resultado con el WebView del sistema.
  \item Los artefactos generados en \texttt{src-tauri/target/release/bundle/} incluyen:
  \begin{itemize}
    \item \texttt{.deb} y \texttt{.AppImage} en Linux.
    \item \texttt{.msi} y \texttt{.exe} (NSIS) en Windows.
    \item \texttt{.dmg} en macOS.
  \end{itemize}
\end{enumerate}
...
A continuación se describen las principales líneas de mejora identificadas para evolucionar el sistema hacia un estándar de calidad corporativo, superando las limitaciones propias de un prototipo académico.

\begin{itemize}
  \item \textbf{Seguridad en WebSockets:} Actualmente, la conexión WebSocket del agente se establece sin un mecanismo de autenticación mutua previo al intercambio de datos. Una mejora prioritaria consiste en implementar un protocolo de autenticación inicial ---por ejemplo, mediante tokens JWT de corta duración o secretos compartidos (\textit{pre-shared keys})--- que obligue al agente a demostrar su identidad antes de que el servidor acepte la conexión. De este modo se mitigaría el riesgo de inyección de telemetría falsa por parte de clientes no autorizados.

  \item \textbf{Validación estricta de certificados SSL/TLS:} Durante la fase de desarrollo, la verificación del certificado del servidor se deshabilitó para facilitar las pruebas en red local con certificados autofirmados. En un entorno de producción real, resulta imprescindible configurar certificados emitidos por una Autoridad Certificadora reconocida y habilitar la validación estricta tanto en el agente como en las conexiones a la API de MikroTik, eliminando así la superficie de ataque frente a ataques de tipo \textit{Man-in-the-Middle} (MitM).

  \item \textbf{Automatización mediante CI/CD:} La incorporación de un \textit{pipeline} de integración y entrega continuas ---por ejemplo, mediante GitHub Actions--- permitiría ejecutar de forma automática las suites de pruebas unitarias, los \textit{linters} de código y los análisis estáticos con cada \textit{push} o \textit{pull request}. Esto garantizaría la detección temprana de regresiones y el mantenimiento de un nivel de calidad de código homogéneo a lo largo de todo el ciclo de vida del proyecto.

  \item \textbf{Empaquetado y despliegue masivo del agente:} Actualmente, el agente se distribuye como un script Python que requiere un intérprete y dependencias instaladas en cada equipo. Una evolución natural consiste en compilar el agente como un ejecutable binario independiente mediante herramientas como PyInstaller y encapsularlo en un instalador silencioso (\texttt{.msi}). Esto posibilitaría el despliegue remoto y transparente en todas las aulas del centro educativo utilizando Directivas de Grupo (GPO) de Windows Server, sin intervención manual en cada equipo.

  \item \textbf{Refinamiento en sistemas UNIX:} En equipos con macOS, la ejecución de determinadas órdenes remotas ---como el apagado del sistema--- requiere privilegios de superusuario. Actualmente, esta operación puede exigir interacción manual para la introducción de credenciales. Una línea de mejora consiste en configurar de forma automatizada el archivo \texttt{sudoers} (mediante \texttt{visudo}) para conceder al usuario del agente permisos específicos sobre los comandos necesarios, permitiendo así una gestión remota completamente desatendida.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Tauri. \textit{Tauri v2 documentation}. \url{https://v2.tauri.app/}
  \item The Rust Programming Language. \textit{The Rust Reference}. \url{https://doc.rust-lang.org/reference/}
  \item PyInstaller. \textit{PyInstaller documentation}. \url{https://pyinstaller.org/en/stable/}
\end{enumerate}
